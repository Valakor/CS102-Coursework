...
template<typename T> T MyList<T>::pop(int loc) {
	if (loc < 0 || loc >= size_) throw std::invalid_argument("mylist.h:68: index out of bounds");
...
template<typename T> T& MyList<T>::at(int loc) const {
	if (loc < 0 || loc >= size_) throw std::invalid_argument("mylist.h:80: index out of bounds");
	return data_[loc];
}
...
template<typename T> T& MyList<T>::operator[](int loc) {
	if (loc < 0 || loc >= size_) throw std::invalid_argument("mylist.h:89: index out of bounds");
	return data_[loc];
}
